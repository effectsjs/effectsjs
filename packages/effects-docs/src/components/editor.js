...
export default class Editor extends React.PureComponent {
  ...
  async componentDidMount () {
    ...
    setTimeout(() => {
      ...
      this.unMonkeyMatch = maybeMonkeyPatchConsole(msg => {
        if (!virtualConsole) virtualConsole = document.getElementById('virtualConsole')
        this.state.logs.push(msg)
        if (this.state.logs.length === 51) this.state.logs.shift()
        this.setState({ logs: [...this.state.logs] }, () => virtualConsole.scrollTo(0, virtualConsole.scrollHeight + 100))
      })
      // blastoff
      if (this.state.src) this.onChange(this.state.src)
    }, 100);
  }
  ...
  onChange = nextSrc => {
    this.setState({ src: nextSrc }, () => {
      if (!this.state.isAutoEval) return
      evaluate(nextSrc)
    })
  }
    ...
    return (
      <div id='editor_container' className={isFullScreen ? 'fullscreen' : ''}>
        ...
        <div id='top_options' className='node'>
          ...
          <input id='toggle_is_fullscreen_control' type='checkbox' name='is_fullscreen' checked={isFullScreen} onChange={this.toggleFullscreen} />
          <label htmlFor='toggle_autoeval_control' children='autoeval' />
          <input id='toggle_autoeval_control' type='checkbox' name='autoeval' checked={isAutoEval} onChange={this.toggleAutoEval} />
        ...
        <AceEditor
          id='editor'
          {...(isFullScreen ? { width: '100%', height: 'calc(85vh - 30px)'} : {width: '100%', height: '400px'})}
        ...
        {logs.map(({ level, timestamp, msg }) => <pre key={timestamp}>[{timestamp}] {level}: {msg}</pre>)}
        </div>
      </div>
    )
  }
...
function maybeMonkeyPatchConsole (onNextMsg) {
  ...
  const testIsPatched = level => level === 'info' || level === 'warn' || level === 'error' || level === 'log'
  ...
  window.console.log = withTap((...args) => appendLog('log', ...args), unpatchedLog);
  window.console.info = withTap((...args) => appendLog('info', ...args), unpatchedInfo);
  window.console.warn = withTap((...args) => appendLog('warn', ...args), unpatchedWarn);
  window.console.error = withTap((...args) => appendLog('error', ...args), unpatchedError);
...
const evaluate = src => {
  ...
  window.requestAnimationFrame(async () => {
    ...
    try {
      ...
      await Promise.resolve(eval(transformed));
    } catch (error) {
      console.error({
      ...
      isEvaluating = false
      if (isRequestingFreshEval) return evaluate(nextToEvaluate)
    }
  })
};
