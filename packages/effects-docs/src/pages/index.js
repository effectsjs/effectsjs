...
import exampleText from 'raw-loader!./index/example-snippet.ts'
...
const IndexPage = () => {
  const {
    ...
    {
      references: markdownRemark(frontmatter: {title: {eq: "index_references"}}) {
        html
      }
      keywords: markdownRemark(frontmatter: {title: {eq: "index_keywords"}}) {
        html
      }
    }
  `)
  return (
    <Layout>
      ...
      <p className='node'>
        Extensions to the javascript language, enabling new control flows
      ...
      <Highlight className='node full-width' language='js' children={exampleText} />
      <p className='node'>
        As seen above, the code 'use effects'
        directive enables the user to begin using effects
        <a className='super' href='#whyuseeffects'>1</a>.
      ...
      <div className='node'>
        ...
        <ul>
          <li children={<Link to='/examples.html'>Try our editor and examples online, now!</Link>} />
          <li children={<Link to='/installation'>Install effects into your javascript or typescript project</Link>} />
        </ul>
      </div>
      <div className='node funsies' children={<Logo />} />
