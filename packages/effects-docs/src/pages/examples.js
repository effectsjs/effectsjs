...
const Examples = () => {
  ...
  return (
    <Layout>
      ...
      <p>
        Want to see effects in action?  Want to edit and tinker with them in real-time?
        You're in luck.  Check out these rad examples:
      </p>
      <table className='node'>
        <tbody>
          {examples.map(({ basename, name }, i) => (
            <tr key={i}>
              <td>{i + 1}</td>
              <td><Link to={`examples/${withoutExampleSuffix(basename.replace('.js', ''))}`}>{name}</Link></td>
            </tr>
          ))}
        </tbody>
      </table>
