...
async function awaitDependencies() {
  ...
  while (true) { // eslint-disable-line
    ...
    const stillWaitingFor = Object.keys(waitingFor).filter(
      key => waitingFor[key] === false
    );
    ...
    await sleep();
    console.info(
      `[dependency-awaiter]: still pending ${stillWaitingFor.join(", ")}`
    );
  }
}
...
function maybeMonkeyPatchConsole(el) {
  ...
  const unpatchedLog = window.console.log;
  const unpatchedInfo = window.console.info;
  const unpatchedWarn = window.console.warn;
  const unpatchedError = window.console.error;
...
async function init() {
  ...
  const evaluate = () => {
    ...
    try {
      const transformed = Babel.transform(text, {
        plugins: [window.transformEffects]
      }).code;
      eval(transformed);
    } catch (error) {
      console.error({
  ...
  // bind handlers
  window.document
    .getElementById("toggle_autoeval_control")
    .addEventListener("input", () => {
      window.effectsjsDocs.isEditorAutoEval = !window.effectsjsDocs
        .isEditorAutoEval;
    });
  window.document
    .getElementById("run_control")
