...
describe('Effect Integrations', () => {
    ...
    test('Execution order of descendant frames', () => {
        ...
        expect(executionOrder).toEqual(
            ['parent start', 'childA start', 'childB call', 'childA end', 'childB call', 'parent end']
        );
    });
    ...
    test('Execution order with handlers', () => {
        ...
        expect(executionOrder).toEqual(
            ['parent pre effect', 'testHandler', 'parent post effect', 'child pre effect', 'testHandler', 'child post effect']
        );
    });
    ...
    test('Async handler timing', (done) => {
        ...
        const program = withHandler(handler, (function*(){
          const then = Date.now();
          yield perform({type : 'testAsync'});
          const now = Date.now();
