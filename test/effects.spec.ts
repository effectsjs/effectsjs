import {stackResume, runProgram, perform, withHandler, InvalidStackFrameError, UnhandledEffectError} from "../src";
import {
    addHandler,
    addReturn,
    Continuation,
...
describe('Effects Unit Tests', () => {
    ...
    describe('stackResume', () => {
       it("Should throw an InvalidStackFrame error upon receiving a non generator input", () => {
           ...
           expect(()=> stackResume(dummyFrame)).toThrowError(InvalidStackFrameError);
       });
       ...
       it(`Should preserve execution order of parent/child frames`, () => {
           ...
           expect(executionOrder).toMatchSnapshot(['parent1', 'child1', 'child2', 'parent2']);
       });
    ...
    describe('perform', () => {
        ...
        it('Should fire arbitrary continuations before passing control back to the control frame', () => {
            ...
            expect(continuationSpy.mock.invocationCallOrder[1]).toBeLessThan(controlFrameSpy.mock.invocationCallOrder[0]);
        });
